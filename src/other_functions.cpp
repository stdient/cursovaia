...
std::string convert(Record record, std::string from_code, std::string to_code) {
    ...
    result += ' ';
    result +=
        convert(std::to_string(record.apartment_number), from_code, to_code);
    result += ' ';
...
void record_converted_values(std::vector<symbol> &symbols) {
    ...
    for (size_t i = 0; i < symbols.size(); ++i) {
        ...
        symbols[i].converted_value =
            std::string(output.data(), outbuf - output.data());
    }
...
std::vector<symbol> find_probabilities(std::string filename) {
    ...
    for (size_t i = 0; i < symbols.size(); ++i) {
        symbols[i].probability =
            std::round(symbols[i].amount_reps / (double)filesize * accurate) /
            accurate;
    }
...
bool isRusLetter(symbol symbol) {
    ...
    for (auto sym : unacceptable_values) {
        if (symbol.converted_value[0] == sym) return false;
    }
    return true;
}
...
void showTable1(std::vector<symbol> symbols) {
    ...
    printLine(63);
    std::cout
        << "|Символ|Вероятность символа|Кодовое слово|Длина кодового слова|"
        << std::endl;
    printLine(63);
    for (size_t i = 0; i < symbols.size(); ++i) {
...
void showTable2(double craft, double entropy, double average_len) {
    std::string buffer;
    std::string titles =
        "|Неравентство Крафта                |Энтропия исходного "
        "текста|Средняя длина кодового "
        "слова|Избыточность кода|";

    int amount_one_byte_symbols_in_title = 14 + 9 + 5;
    int devided_line_size =
        (titles.size() - amount_one_byte_symbols_in_title) / 2 +
        amount_one_byte_symbols_in_title;
...
double find_symbols_sum(std::vector<symbol> symbols,
                        double (*function)(std::vector<symbol> symbols,
                                           int i)) {
    double sum = 0;
    for (size_t i = 0; i < symbols.size(); ++i) {
...
void sort(std::vector<symbol> &arr, int L, int R) {
    ...
    if (idx_current_left < R) sort(arr, idx_current_left, R);
    if (L < idx_current_right) sort(arr, L, idx_current_right);
}

void task(std::vector<symbol> &symbols) {
    for (size_t i = 0; i < symbols.size(); ++i) symbols[i].reset();
...
int findInTree(Tree<Record> &tree, int amount_keys, struct termios old,
               struct termios cur) {
    system("clear");
...
int menuLoop(Queue database, Node **database_frames, Database_settings param) {
    ...
    while (1) {
        ...
        switch (action) {
            ...
            case '5':
                if (buildTree(keys, amount_keys, KDI, tree)) break;
                tree.display();
                getchar();
                break;
...
void showMenu() {
    ...
    std::string menuSection[COUNT_OF_SECTIONS] = {
        ...
        "Поиск по ключу",      "Построить дерево",    "Поиск в дереве",
        "Закодировать файл"};
...
void printDData(Record data) {
    printf("%s | %s | %4d | %4d | %s\n", data.citizen_full_name,
           data.street_name, data.house_number, data.apartment_number,
